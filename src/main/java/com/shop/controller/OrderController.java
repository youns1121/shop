...
public class OrderController {
    ...
    public ResponseEntity<String> order (@Valid @RequestBody OrderDto orderDto, BindingResult bindingResult, Principal principal) {
        ...
        if(bindingResult.hasErrors()){
            ...
            return new ResponseEntity<>(sb.toString(), HttpStatus.BAD_REQUEST); // 에러 정보를 ResponseEntity 객체에 담아서 반환
        }
        ...
        return new ResponseEntity<>(orderId.toString(), HttpStatus.OK); // 결과값으로 생성된 주문 번호와 요청이 성공했다는 HTTP응답 상 태 코드를 반환
        }
